...
import {
  ...
  X,
  AlertTriangle,
  GripVertical, // Icon dùng làm tay cầm kéo thả
} from "lucide-react";
...
export default function SubjectPage() {
  ...
  // 3️⃣ State Xóa Module
  const [showDeleteModal, setShowDeleteModal] = useState<boolean>(false);
  const [deletingModuleId, setDeletingModuleId] = useState<string | null>(null);
  const [deleting, setDeleting] = useState<boolean>(false);

  // States & Ref quản lý Syllabus (Đề cương)
  ...
  // Tải danh sách Module (Đã sắp xếp theo order_index)
  ...
  // -------------------------------------------------------------
  // 🖐️ XỬ LÝ KÉO THẢ (DRAG & DROP) MODULES
  // -------------------------------------------------------------
  ...
  const handleDragOver = (e: React.DragEvent, targetIndex: number) => {
    ...
    // Cập nhật lại order_index theo thứ tự mới (1-based index)
  ...
  const handleDragEnd = async () => {
    ...
    try {
      ...
      fetchModules(); // Reset lại nếu lỗi
    } finally {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = uploadRes.file_path || uploadRes.path || uploadRes.url;

      if (syllabus?.syllabus_id) {
        await updateSyllabusAction(syllabus.syllabus_id, {
          syllabus_file_path: filePath,
        });
      } else {
        await createSyllabusAction({
          subject_id: subjectId,
          description: `Đề cương môn học ${subjectId}`,
          syllabus_file_path: filePath,
      ...
      alert("Lỗi khi cập nhật đề cương: " + (err.message || "Lỗi hệ thống"));
    } finally {
  ...
  const handleCreateModule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createModuleAction({
        ...
        title: newTitle,
        order_index: modules.length + 1, // Đặt thứ tự tự động ở cuối
      });
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex justify-between items-center border-b border-slate-100 pb-4">
            ...
            {syllabus && (
              <span
                className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full border ${syllabusStatus.className}`}
              >
                <StatusIcon size={14} />
                {syllabusStatus.text}
              </span>
          ...
          {loadingSyllabus ? (
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                <button
                  ...
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 text-xs font-semibold transition disabled:opacity-50"
                >
                  {downloadingSyllabus ? (
                  ...
                  {downloadingSyllabus ? "Đang tải..." : "Tải về"}
                </button>

                <button
                  disabled={uploadingSyllabus}
                  onClick={() => fileInputRef.current?.click()}
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 text-xs font-semibold transition disabled:opacity-50"
                >
                  {uploadingSyllabus ? (
                  ...
                  {uploadingSyllabus ? "Đang tải lên..." : "Cập nhật File"}
                </button>
              </div>
            </div>
          ) : (
            <div className="bg-slate-50 border border-dashed border-slate-200 rounded-xl p-8 text-center flex flex-col items-center justify-center gap-3">
              ...
              <button
                disabled={uploadingSyllabus}
                onClick={() => fileInputRef.current?.click()}
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition shadow-sm"
              >
                {uploadingSyllabus ? (
                  <Loader2 size={15} className="animate-spin" />
                ) : (
        ...
        {/* Danh sách Modules (Có kéo thả reorder) */}
        <div className="space-y-4">
