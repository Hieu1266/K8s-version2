        ...
        if question.rubrics:
            for rub in question.rubrics:
                db_rubric = RubricCriteria(
                    ...
                    max_score=float(rub.percentage or 0.0)
                )
                db.add(db_rubric)
...
def update_question(
    ...
    try:
        # 1. Cập nhật trực tiếp thông tin câu hỏi (loại bỏ trường 'rubrics')
        ...
        # 2. Cập nhật lại danh sách Rubrics
        ...
        db.commit() # Flush & Commit toàn bộ xuống PostgreSQL
